...
import { maybeAddHeader, maybeAddQueryParameter } from "../Requests.js";
 ...
 * @category Services
 */
export const make: Effect.Effect<PluginsImpl, never, IMobyConnectionAgent | HttpClient.HttpClient.Default> = Effect.gen(
    function* () {
        const agent = yield* MobyConnectionAgent;
        const defaultClient = yield* HttpClient.HttpClient;

        const client = defaultClient.pipe(
            HttpClient.mapRequest(HttpClientRequest.prependUrl(`${agent.nodeRequestUrl}/plugins`)),
        ...
        const PluginsClient = client.pipe(
            HttpClient.mapEffect(HttpClientResponse.schemaBodyJson(Schema.Array(Plugin)))
        );
