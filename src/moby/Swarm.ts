 ...
 * @category Services
 */
export const make: Effect.Effect<Swarms, never, IMobyConnectionAgent | HttpClient.HttpClient.Default> = Effect.gen(
    function* () {
        const agent = yield* MobyConnectionAgent;
        const defaultClient = yield* HttpClient.HttpClient;

        const client = defaultClient.pipe(
            HttpClient.mapRequest(HttpClientRequest.prependUrl(`${agent.nodeRequestUrl}/swarm`)),
        ...
        const unlock_ = (
            options: Schema.Schema.Type<typeof SwarmUnlockRequest>
        ): Effect.Effect<void, SwarmsError, never> =>
            Function.pipe(
 ...
 * @category Layers
 */
export const layer: Layer.Layer<Swarms, never, IMobyConnectionAgent> = Layer.effect(Swarms, make).pipe(
...
): Layer.Layer<Swarms, never, Scope.Scope> => layer.pipe(Layer.provide(Layer.effect(MobyConnectionAgent, agent)));
...
): Layer.Layer<Swarms, never, Scope.Scope> => fromAgent(getAgent(connectionOptions));
