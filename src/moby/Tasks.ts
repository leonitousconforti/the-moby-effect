 ...
 * @category Services
 */
export const make: Effect.Effect<TasksImpl, never, IMobyConnectionAgent | HttpClient.HttpClient.Default> = Effect.gen(
    function* () {
        const agent = yield* MobyConnectionAgent;
        const defaultClient = yield* HttpClient.HttpClient;

        const client = defaultClient.pipe(
            HttpClient.mapRequest(HttpClientRequest.prependUrl(`${agent.nodeRequestUrl}/tasks`)),
        ...
        ): Effect.Effect<Readonly<Array<Task>>, TasksError, never> =>
            Function.pipe(
