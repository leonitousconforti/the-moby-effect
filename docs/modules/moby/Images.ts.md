...
) => Layer.Layer<Images, never, Scope.Scope>
```
...
```ts
export declare const make: Effect.Effect<ImagesImpl, never, IMobyConnectionAgent | HttpClient.HttpClient.Default>
```
...
```ts
export declare const Images: Context.Tag<Images, ImagesImpl>
```

Added in v1.0.0

## ImagesImpl (interface)
